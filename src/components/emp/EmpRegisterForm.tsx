 ...
 *
 * 역할(ROLE)은 이 폼에서 받지 않는다. 직원을 먼저 등록하고 수정 폼에서 배정한다.
 * payload 에 roleIds 키를 아예 넣지 않아야(빈 배열 [] 이 아니라 생략)
 * 서버가 EMP_ROLE 을 건드리지 않고 그냥 넘어간다.
 */
...
export default function EmpRegisterForm({
  ...
  function buildPayload(): EmpRegisterRequest {
    return {
      ...
      // roleIds 와 assignedBy 는 일부러 넣지 않는다 (역할은 수정 폼에서 배정).
